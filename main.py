...
async def create_all_protocols_sub(
    request: Request,
    *,
    label: str,
    category_id: str = "0",
    ...
    note: str = "",
    fingerprint: str = DEFAULT_FINGERPRINT,
    alpn_by_protocol: dict | None = None,
    port: int = DEFAULT_PORT,
    ip_limit: int = 0,
    speed_limit_bytes: int = 0,
    connection_limit: int = 0,
    ...
    """Create every supported protocol as separate links under one subscription."""
    base_label = sanitize_config_name((label or "لینک جدید").strip() or project_config_name())
    sub_id, sub = await create_sub_group(
        name=base_label[:60],
        desc="ساخت همه پروتکل‌ها باهم",
    )

    if isinstance(clean_ips, list):
        normalized_clean_ips = [str(x).strip() for x in clean_ips if str(x).strip()]
    else:
        normalized_clean_ips = [x.strip() for x in str(clean_ips or "").replace(",", "\n").splitlines() if x.strip()]
    ...
    for proto in PROTOCOLS:
        ...
        uid, link = await make_link(
            ...
            alpn=(alpn_by_protocol or {}).get(proto, DEFAULT_ALPN_BY_PROTOCOL.get(proto, "")),
            ...
            fragment=cfg.get("fragment", fragment),
            clean_ips=cfg.get("clean_ips", normalized_clean_ips),
            alarm_enabled=bool(cfg.get("alarm_enabled", alarm_enabled)),
    ...
    # IMPORTANT: when "all protocols" is enabled, the returned subscription
    # must point to the shared group subscription, not the first protocol's
    # individual /sub/{uuid} URL.  The group endpoint aggregates every
    # protocol link into one subscription.
    shared_sub_url = f"https://{host}/sub-group/{sub['uuid_key']}"
    shared_public_url = f"https://{host}/public-sub/{sub['uuid_key']}"
    result = {
        **get_link_info(first_link, first_uid, host),
        "ok": True,
        "all_protocols": True,
        "created_protocols": len(created),
        "created_links": [get_link_info(link, uid, host) for uid, link in created],
        "sub_id": sub_id,
        # Expose the shared subscription as the primary subscription URL.
        "sub": shared_sub_url,
        "sub_url": shared_sub_url,
        "public_url": shared_public_url,
    }
    log_activity("sub", f"ساخت همه پروتکل‌ها باهم برای «{base_label}» ({len(created)} پروتکل)", "ok")
    return result
...
async def create_link_api(
    ...
    # When enabled, create every supported protocol inside one shared subscription.
    if bool(body.get("all_protocols", False)):
        ...
        if cat.get("connection_limit") and safe_int(body.get("connection_limit", 0), minimum=0) <= 0:
            body["connection_limit"] = int(cat["connection_limit"])
        if cat.get("ip_limit") and safe_int(body.get("ip_limit", 0), minimum=0) <= 0:
            body["ip_limit"] = int(cat["ip_limit"])
        ...
        if cat.get("speed_limit_bytes") and speed_bytes <= 0: speed_bytes = int(cat["speed_limit_bytes"])
        label_val = body.get("label", "")
        if cat.get("random_name") or not str(label_val).strip(): label_val = project_config_name()
        else: label_val = sanitize_config_name(str(label_val))
        return await create_all_protocols_sub(
            request, label=label_val, category_id=category_id, config_count=config_count,
            limit_bytes=limit_bytes, expires_at=expires_at, note=body.get("note", ""),
            ...
            fragment=str(body.get("fragment", "off") or "off"),
            clean_ips=body.get("clean_ips") or body.get("clean_ip") or "",
...
<section class="page" id="page-create">
  ...
  <div class="g2">
    <div class="card">
      ...
      <div class="all-protocol-toggle" role="switch" aria-checked="false" tabindex="0" onclick="toggleAllProtocols('manual')" onkeydown="if(event.key==='Enter'||event.key===' '){event.preventDefault();toggleAllProtocols('manual')}" >
        <div class="all-protocol-copy"><b>ساخت همه پروتکل‌ها باهم</b><span>همه پروتکل‌های موجود در یک ساب ساخته می‌شوند.</span></div>
        <span class="all-protocol-switch" id="allProtocolsManual" aria-hidden="true"><i></i></span>
      </div>
...
<div class="form-row">
    ...
    <div class="card" style="border-color:rgba(139,92,246,.35)">
      ...
      <div class="all-protocol-toggle" role="switch" aria-checked="false" tabindex="0" onclick="toggleAllProtocols('auto')" onkeydown="if(event.key==='Enter'||event.key===' '){event.preventDefault();toggleAllProtocols('auto')}" >
        <div class="all-protocol-copy"><b>ساخت همه پروتکل‌ها باهم</b><span>همه پروتکل‌های موجود در یک ساب ساخته می‌شوند.</span></div>
        <span class="all-protocol-switch" id="allProtocolsAuto" aria-hidden="true"><i></i></span>
      </div>
...
.all-protocol-toggle{display:flex;align-items:center;justify-content:space-between;gap:14px;padding:13px 15px;margin:0 0 16px;border:1px solid rgba(43,155,255,.34);border-radius:18px;background:linear-gradient(135deg,rgba(18,54,96,.58),rgba(5,21,43,.68));box-shadow:inset 0 1px 0 rgba(255,255,255,.07),0 8px 24px rgba(0,80,180,.08);cursor:pointer;transition:.2s ease}
.all-protocol-toggle:hover{border-color:rgba(52,177,255,.62);transform:translateY(-1px)}
.all-protocol-copy{min-width:0;flex:1;direction:rtl;text-align:right}.all-protocol-copy b{display:block;color:var(--t1);font-size:14px;margin-bottom:4px}.all-protocol-copy span{display:block;color:var(--t2);font-size:11px;line-height:1.6}
.all-protocol-switch{width:54px;height:30px;flex:0 0 54px;border-radius:999px;background:rgba(95,105,120,.48);border:1px solid rgba(255,255,255,.12);padding:3px;transition:.22s ease;position:relative;direction:ltr}.all-protocol-switch i{display:block;width:22px;height:22px;border-radius:50%;background:#e8edf4;box-shadow:0 2px 8px rgba(0,0,0,.35);transition:.22s ease}.all-protocol-toggle.on{border-color:rgba(34,230,126,.72);box-shadow:inset 0 1px 0 rgba(255,255,255,.08),0 0 24px rgba(26,220,113,.13)}.all-protocol-toggle.on .all-protocol-switch{background:linear-gradient(135deg,#19d879,#0ca95a);box-shadow:0 0 16px rgba(27,218,117,.32)}.all-protocol-toggle.on .all-protocol-switch i{transform:translateX(24px)}
@media(max-width:600px){.all-protocol-toggle{padding:11px 12px;border-radius:15px;margin-bottom:13px}.all-protocol-copy b{font-size:12px}.all-protocol-copy span{font-size:9px}.all-protocol-switch{width:48px;height:27px;flex-basis:48px}.all-protocol-switch i{width:19px;height:19px}.all-protocol-toggle.on .all-protocol-switch i{transform:translateX(21px)}}
</style>
...
function getSubUrl(l){if(!l)return '';return l.sub_url||l.sub||l.info||''}
...
function toggleAllProtocols(mode){
  __allProtocolMode[mode]=!__allProtocolMode[mode];
  const id=mode==='manual'?'allProtocolsManual':'allProtocolsAuto';
  const sw=document.getElementById(id);
  const root=sw?.closest('.all-protocol-toggle');
  if(root){root.classList.toggle('on',__allProtocolMode[mode]);root.setAttribute('aria-checked',__allProtocolMode[mode]?'true':'false')}
  const protoField=document.querySelector(mode==='manual'?'[data-protocol-picker="cProto"]':'[data-protocol-picker="aProto"]');
  if(protoField){protoField.style.opacity=__allProtocolMode[mode]?'0.48':'1';protoField.style.pointerEvents=__allProtocolMode[mode]?'none':'auto';}
}
