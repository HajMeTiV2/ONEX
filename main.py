...
@media (max-width:768px){
  ...
  .dashboard-hero{width:100%;margin-bottom:14px;gap:8px;align-items:stretch;grid-template-columns:minmax(0,1fr);grid-template-areas:"hero" "actions"}
  .dashboard-hero .hero-main{grid-area:hero;align-self:center}.dashboard-hero .hero-actions{grid-area:actions;width:100%;justify-content:stretch}
...
.onex-version-strip{display:flex;align-items:center;justify-content:space-between;gap:10px;min-width:0;padding:9px 13px!important;border:1px solid rgba(59,130,246,.18);border-radius:18px;background:linear-gradient(135deg,rgba(8,25,48,.88),rgba(4,13,28,.78));box-shadow:inset 0 1px rgba(255,255,255,.06),0 10px 28px rgba(0,0,0,.18)}.version-item{display:flex;align-items:center;gap:7px;min-width:0}.version-item>div{display:flex;flex-direction:column;min-width:0}.version-item small{font-size:8px;color:var(--t3);line-height:1.1;white-space:nowrap}.version-item b{font-size:11px;color:var(--t1);line-height:1.35;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;max-width:190px}.version-current b{color:#67e8f9}.version-icon{display:flex;align-items:center;justify-content:center;width:26px;height:26px;border-radius:9px;background:rgba(37,99,235,.13);color:#60a5fa;font-size:15px;flex:0 0 26px}.onex-version-strip .top-sep{height:30px!important;flex:0 0 1px!important;background:rgba(148,163,184,.14)!important}.onex-version-strip .top-dot{width:8px!important;height:8px!important;flex:0 0 8px!important}.top-setting-group{display:flex;align-items:center;gap:3px;padding:3px;border:1px solid var(--card-b);border-radius:11px;background:rgba(255,255,255,.025);box-shadow:inset 0 1px rgba(255,255,255,.04)}
...
<div class="onex-topbar">
  <div class="top-server onex-version-strip">
    <div class="version-item version-current"><span class="top-dot"></span><div><small data-i18n="current_version">ورژن فعلی</small><b id="topCurrentVersion">v1.0.1</b></div></div>
    <span class="top-sep"></span>
    <div class="version-item"><span class="version-icon">◉</span><div><small>دامنه</small><b id="topHost">—</b></div></div>
    <span class="top-sep"></span>
    <div class="version-item"><span class="version-icon">◷</span><div><small>Uptime</small><b id="topUptime">—</b></div></div>
    <span class="top-sep"></span>
    <div class="version-item"><span class="version-icon">✓</span><div><small>وضعیت</small><b>آنلاین</b></div></div>
  </div>
...
/* FINAL MOBILE TOP SERVER BAR — clean two-row glass layout */
@media (max-width:768px){
  .onex-topbar{
    width:100%!important;
  ...
  .onex-topbar .top-server{
    ...
    border:1px solid rgba(37,170,255,.24)!important;
    border-radius:15px!important;
    background:linear-gradient(135deg,rgba(8,27,53,.94),rgba(3,12,27,.86))!important;
    box-shadow:inset 0 1px rgba(255,255,255,.07),0 8px 24px rgba(0,0,0,.22)!important;
  }
  .onex-topbar .version-item{
  ...
  .onex-topbar .version-item>div{
    min-width:0!important;
    max-width:100%!important;
    align-items:flex-start!important;
  }
  .onex-topbar .version-item small{
  ...
  .onex-topbar .version-icon{
    width:25px!important;height:25px!important;flex:0 0 25px!important;
    border-radius:8px!important;font-size:13px!important;
  }
  .onex-topbar .top-dot{width:7px!important;height:7px!important;flex:0 0 7px!important}
  .onex-topbar .top-sep{height:30px!important;flex:0 0 1px!important;margin:0 1px!important}
  .onex-topbar .top-setting-group{height:34px!important;flex:0 0 auto!important}
  .onex-topbar .top-setting-btn{font-size:8px!important;padding:5px 9px!important}
  .onex-topbar .top-notify-btn{width:34px!important;height:34px!important;padding:0!important;display:flex!important;align-items:center!important;justify-content:center!important}
  .onex-topbar .top-notify-btn .notify-label{display:none!important}
  .onex-topbar .top-notify-btn .notify-badge{right:-2px!important;top:-3px!important}
  .onex-topbar .top-avatar{width:34px!important;height:34px!important;flex:0 0 34px!important}
}
@media (max-width:420px){
  .onex-topbar .version-item{gap:3px!important;padding:0 2px!important}
  .onex-topbar .version-icon{width:22px!important;height:22px!important;flex-basis:22px!important;font-size:11px!important}
  .onex-topbar .version-item small{font-size:6.5px!important}
  .onex-topbar .version-item b{font-size:9px!important}
}
@media (max-width:360px){
  .onex-topbar{padding:6px!important;gap:6px!important}
  .onex-topbar .version-item{gap:2px!important}
  .onex-topbar .version-icon{width:20px!important;height:20px!important;flex-basis:20px!important;font-size:10px!important}
  .onex-topbar .version-item small{font-size:6px!important}
  .onex-topbar .version-item b{font-size:8px!important}
}
</style>
...
async function refreshAll(){
  ...
  const upEl=document.getElementById('topUptime'); const mu=document.getElementById('mUptime'); if(upEl && mu) upEl.textContent=mu.textContent||'—';
  const tv=document.getElementById('topCurrentVersion'); if(tv){ const ver=(__updateInfo&&__updateInfo.current_version)||'1.0.1'; tv.textContent='v'+String(ver).replace(/^v/i,''); }
}
...
async function checkPanelUpdateWithNotify(showToast=false){if(__updateCheckBusy)return __updateInfo;__updateCheckBusy=true;try{const r=await api('/api/update/check');if(r&&r.ok){const old=__updateInfo&&__updateInfo.latest_version;__updateInfo=r;setVersionLabels(r.current_version||'1.0.1',r.latest_version||r.current_version);const tv=document.getElementById('topCurrentVersion');if(tv)tv.textContent='v'+String(r.current_version||'1.0.1').replace(/^v/i,'');renderNotifications();if(r.update_available&&showToast&&old!==r.latest_version)toast(updateText(`نسخه جدید ${r.latest_version} آماده است`,`Version ${r.latest_version} is available`));}return r}catch(e){return null}finally{__updateCheckBusy=false}}
