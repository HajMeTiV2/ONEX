...
function closeConfigMenus(){document.querySelectorAll('.cfg-menu.open').forEach(m=>{m.classList.remove('open');m.style.left='';m.style.top='';})}
function toggleConfigMenu(e,uid){e.preventDefault();e.stopPropagation();const id='cfgMenu_'+uid.replace(/[^a-zA-Z0-9_-]/g,'_'),m=document.getElementById(id);if(!m)return;const was=m.classList.contains('open');closeConfigMenus();if(was)return;const r=e.currentTarget.getBoundingClientRect();m.classList.add('open');const gap=7,w=Math.min(184,window.innerWidth-16),mh=Math.min(320,window.innerHeight-20);m.style.width=w+'px';m.style.maxHeight=mh+'px';let left=r.right-w;if(left<8)left=8;if(left+w>window.innerWidth-8)left=window.innerWidth-w-8;let top=r.bottom+gap;const h=Math.min(m.scrollHeight||mh,mh);if(top+h>window.innerHeight-8)top=Math.max(8,r.top-gap-h);m.style.left=Math.round(left)+'px';m.style.top=Math.round(top)+'px';requestAnimationFrame(()=>{if(m.classList.contains('open')){const rr=m.getBoundingClientRect();if(rr.bottom>window.innerHeight-8)m.style.top=Math.max(8,window.innerHeight-8-rr.height)+'px';}})}
window.addEventListener('resize',closeConfigMenus,{passive:true});window.addEventListener('scroll',closeConfigMenus,{passive:true,capture:true});
