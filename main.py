...
const PROTOCOL_PICKER_ICONS={
  'vless-ws':['V','#22d3ee','#2563eb'],
  'xhttp-packet-up':['P','#34d399','#059669'],
  'xhttp-stream-up':['S','#fbbf24','#d97706'],
  'xhttp-stream-one':['C','#a78bfa','#7c3aed'],
  'vmess-ws':['M','#818cf8','#4f46e5'],
  'trojan-ws':['T','#fb7185','#e11d48'],
  'shadowsocks':['S','#a3e635','#65a30d'],
  'socks5':['5','#38bdf8','#0284c7'],
  'http':['H','#cbd5e1','#64748b'],
  'hysteria2':['H2','#22d3ee','#0e7490'],
  'tuic':['T','#fb923c','#c2410c'],
  'wireguard':['W','#60a5fa','#2563eb'],
  'highspeed-demo':['T','#f472b6','#db2777'],
  'gaming-lite-demo':['G','#c084fc','#9333ea']
};
...
function protocolIconMarkup(id){
  const d=PROTOCOL_PICKER_ICONS[id]||PROTOCOL_PICKER_ICONS['vless-ws'];
  const [letter,c1,c2]=d;
  const safe=String(letter).replace(/[<>&"']/g,'');
  return `<span class="protocol-option-icon proto3d" aria-hidden="true"><svg class="proto3d-svg" viewBox="0 0 86 86" role="img">
    <defs>
      <linearGradient id="g_${id}" x1="0" y1="0" x2="1" y2="1"><stop offset="0" stop-color="${c1}"/><stop offset=".52" stop-color="${c2}"/><stop offset="1" stop-color="#111827"/></linearGradient>
      <linearGradient id="top_${id}" x1="0" y1="0" x2="1" y2="1"><stop offset="0" stop-color="#fff" stop-opacity=".72"/><stop offset="1" stop-color="${c1}" stop-opacity=".45"/></linearGradient>
      <filter id="sh_${id}" x="-60%" y="-60%" width="220%" height="220%"><feDropShadow dx="0" dy="7" stdDeviation="5" flood-opacity=".38"/></filter>
    </defs>
    <ellipse cx="43" cy="72" rx="18" ry="4" fill="#020617" opacity=".45"/>
    <g filter="url(#sh_${id})">
      <path d="M25 28 47 18 66 29 44 40Z" fill="url(#top_${id})"/>
      <path d="M25 28 44 40 44 66 25 55Z" fill="${c2}" opacity=".92"/>
      <path d="M44 40 66 29 66 55 44 66Z" fill="url(#g_${id})"/>
      <path d="M25 28 47 18 66 29 44 40Z" fill="url(#top_${id})" opacity=".8"/>
      <text x="45" y="54" text-anchor="middle" font-family="Arial,sans-serif" font-size="17" font-weight="900" fill="#fff">${safe}</text>
    </g>
    <ellipse cx="45" cy="41" rx="29" ry="10" fill="none" stroke="#e2e8f0" stroke-opacity=".62" stroke-width="1.5" transform="rotate(-18 45 41)"/>
    <circle cx="70" cy="32" r="3" fill="#fff" opacity=".85"/>
    <circle cx="20" cy="51" r="2" fill="${c1}" opacity=".7"/>
  </svg></span>`;
}
