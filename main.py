...
function protocolIconMarkup(id){
  ...
  const palettes={
    'vless-ws':['#67e8f9','#2563eb','#312e81'],'xhttp-packet-up':['#86efac','#059669','#064e3b'],
    'xhttp-stream-up':['#fde68a','#f59e0b','#92400e'],'xhttp-stream-one':['#ddd6fe','#7c3aed','#3b0764'],
    'vmess-ws':['#bfdbfe','#4f46e5','#1e1b4b'],'trojan-ws':['#fecdd3','#e11d48','#4c0519'],
    'shadowsocks':['#d9f99d','#65a30d','#365314'],'socks5':['#7dd3fc','#0284c7','#082f49'],
    'http':['#f8fafc','#64748b','#1e293b'],'hysteria2':['#a5f3fc','#0891b2','#164e63'],
    'tuic':['#fed7aa','#f97316','#7c2d12'],'wireguard':['#dbeafe','#2563eb','#172554'],
    'highspeed-demo':['#fef08a','#eab308','#713f12'],'gaming-lite-demo':['#f5d0fe','#a855f7','#581c87']
  };
  const [a,b,c]=palettes[id]||['#93c5fd','#2563eb','#1e3a8a'];
  const safe=String(glyph).replace(/[&<>"']/g,m=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[m]));
  return `<span class="protocol-option-icon proto-3d" aria-hidden="true"><svg class="static-icon" viewBox="0 0 64 64" xmlns="http://www.w3.org/2000/svg"><defs><linearGradient id="g-${id}" x1="0" y1="0" x2="1" y2="1"><stop offset="0" stop-color="${a}"/><stop offset=".55" stop-color="${b}"/><stop offset="1" stop-color="${c}"/></linearGradient></defs><ellipse cx="32" cy="55" rx="18" ry="4" fill="${b}" opacity=".20"/><path d="M16 20 40 11 51 22 27 31Z" fill="#fff" opacity=".22"/><path d="M16 20v27l11 8V31Z" fill="${c}"/><path d="M27 31v24l24-10V22Z" fill="url(#g-${id})"/><path d="M16 20 40 11 51 22 27 31Z" fill="url(#g-${id})"/><path d="M23 24 39 18 44 23 28 29Z" fill="#fff" opacity=".16"/><text x="33" y="42" text-anchor="middle" fill="#fff" font-family="Arial,sans-serif" font-size="${safe.length>1?9:15}" font-weight="800">${safe}</text><path d="M9 34c5-12 19-18 34-14 7 2 12 7 13 13" fill="none" stroke="#fff" stroke-opacity=".45" stroke-width="1.4"/><circle cx="54" cy="25" r="2.3" fill="#fff" opacity=".8"/></svg></span>`;
}
