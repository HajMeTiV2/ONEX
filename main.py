...
/* ============================================================
   STATIC 3D PROTOCOL ICONS — lightweight, no animation
   ============================================================ */
.protocol-option-icon.proto-3d{width:58px;height:58px;display:grid;place-items:center;position:relative;flex:0 0 auto;z-index:2}
.proto-3d .static-icon{width:56px;height:56px;display:block;filter:drop-shadow(0 7px 10px rgba(0,0,0,.28))}
.protocol-option:hover .static-icon{filter:drop-shadow(0 9px 13px rgba(37,99,235,.24))}
.protocol-option.selected .static-icon{filter:drop-shadow(0 9px 15px rgba(56,189,248,.35))}
...
function protocolIconMarkup(id){
  const palettes={
    'vless-ws':['#8be9ff','#2563eb','#312e81'],
    'xhttp-packet-up':['#a7f3d0','#10b981','#064e3b'],
    'xhttp-stream-up':['#fde68a','#f59e0b','#7c2d12'],
    'xhttp-stream-one':['#ddd6fe','#7c3aed','#3b0764'],
    'vmess-ws':['#bfdbfe','#4f46e5','#1e1b4b'],
    'trojan-ws':['#fecdd3','#e11d48','#4c0519'],
    'shadowsocks':['#d9f99d','#65a30d','#365314'],
    'socks5':['#7dd3fc','#0284c7','#082f49'],
    'http':['#e2e8f0','#64748b','#1e293b'],
    'hysteria2':['#a5f3fc','#0891b2','#164e63'],
    'tuic':['#fed7aa','#f97316','#7c2d12'],
    'wireguard':['#dbeafe','#2563eb','#172554'],
    'highspeed-demo':['#fef08a','#eab308','#713f12'],
    'gaming-lite-demo':['#f5d0fe','#a855f7','#581c87']
  };
  const glyphs={
    'vless-ws':'V','xhttp-packet-up':'P','xhttp-stream-up':'S','xhttp-stream-one':'C',
    'vmess-ws':'M','trojan-ws':'T','shadowsocks':'S','socks5':'5','http':'H',
  ...
  const [a,b,c]=palettes[id]||['#93c5fd','#2563eb','#1e3a8a'];
  const glyph=glyphs[id]||'◆';
  const gid='neo-'+String(id).replace(/[^a-zA-Z0-9_-]/g,'-');
  const safe=String(glyph).replace(/[&<>"']/g,m=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[m]));
  const shapes={
    'vless-ws':`<path d="M31 8 48 18 44 43 30 52 16 43 12 18Z" fill="url(#${gid})"/><path d="M31 8v44M12 18l19 10 17-10M16 43l15-15 13 15" fill="none" stroke="#fff" stroke-opacity=".28" stroke-width="1"/>`,
    'xhttp-packet-up':`<path d="M20 17 43 9 52 20 29 30 12 25Z" fill="url(#${gid})"/><path d="M12 25v22l17 9V30Z" fill="${c}"/><path d="M29 30v26l23-12V20Z" fill="url(#${gid})"/><path d="M20 17 43 9 52 20 29 30Z" fill="#fff" opacity=".16"/>`,
    'xhttp-stream-up':`<path d="M32 7 48 25 38 27 42 53 18 32 28 30 23 9Z" fill="url(#${gid})"/><path d="M28 30 38 27" stroke="#fff" stroke-opacity=".55" stroke-width="1.5"/>`,
    'xhttp-stream-one':`<path d="M31 8 49 20 43 46 31 55 18 46 13 20Z" fill="url(#${gid})"/><path d="M31 8v47M13 20l18 10 18-10M18 46l13-16 12 16" fill="none" stroke="#fff" stroke-opacity=".30" stroke-width="1"/>`,
    'vmess-ws':`<path d="M32 8 50 19 45 44 32 54 19 44 14 19Z" fill="url(#${gid})"/><path d="M19 44 32 30 45 44M14 19l18 11 18-11" fill="none" stroke="#fff" stroke-opacity=".32" stroke-width="1"/>`,
    'trojan-ws':`<path d="M31 7 51 18 45 45 31 54 17 45 11 18Z" fill="url(#${gid})"/><path d="M17 45 31 30 45 45M11 18l20 12 20-12" fill="none" stroke="#fff" stroke-opacity=".3" stroke-width="1"/>`,
    'shadowsocks':`<path d="M31 9 48 19 43 45 31 53 18 45 13 19Z" fill="url(#${gid})"/><path d="M18 45 31 31 43 45M13 19l18 12 17-12" fill="none" stroke="#fff" stroke-opacity=".28" stroke-width="1"/>`,
    'socks5':`<path d="M31 8 49 18 45 44 31 54 17 44 13 18Z" fill="url(#${gid})"/><path d="M31 8v46M13 18l18 12 18-12" fill="none" stroke="#fff" stroke-opacity=".3" stroke-width="1"/>`,
    'http':`<path d="M31 8 49 18 44 45 31 54 18 45 13 18Z" fill="url(#${gid})"/><path d="M31 8v46M13 18l18 12 18-12M18 45l13-15 13 15" fill="none" stroke="#fff" stroke-opacity=".34" stroke-width="1"/>`,
    'hysteria2':`<path d="M31 7 50 20 42 45 31 55 20 45 12 20Z" fill="url(#${gid})"/><path d="M31 7v48M12 20l19 11 19-11M20 45l11-14 11 14" fill="none" stroke="#fff" stroke-opacity=".32" stroke-width="1"/>`,
    'tuic':`<path d="M31 8 50 19 43 46 31 54 18 46 12 19Z" fill="url(#${gid})"/><path d="M31 8v46M12 19l19 11 19-11" fill="none" stroke="#fff" stroke-opacity=".3" stroke-width="1"/>`,
    'wireguard':`<circle cx="32" cy="32" r="22" fill="url(#${gid})"/><path d="M32 10v44M10 32h44" stroke="#fff" stroke-opacity=".18" stroke-width="1"/><path d="M17 43 32 27 47 43" fill="none" stroke="#fff" stroke-opacity=".28" stroke-width="1"/>`,
    'highspeed-demo':`<path d="M31 7 51 20 42 47 31 55 20 47 11 20Z" fill="url(#${gid})"/><path d="M31 7v48M11 20l20 11 20-11" fill="none" stroke="#fff" stroke-opacity=".34" stroke-width="1"/>`,
    'gaming-lite-demo':`<path d="M31 7 49 19 45 44 31 54 17 44 13 19Z" fill="url(#${gid})"/><path d="M17 44 31 30 45 44M13 19l18 11 18-11" fill="none" stroke="#fff" stroke-opacity=".34" stroke-width="1"/>`
  };
  const shape=shapes[id]||shapes['vless-ws'];
  const orbit=id==='wireguard'
    ? `<ellipse cx="32" cy="32" rx="27" ry="10" transform="rotate(-28 32 32)" fill="none" stroke="#fff" stroke-opacity=".58" stroke-width="1.4"/>`
    : `<ellipse cx="32" cy="31" rx="28" ry="10" transform="rotate(-24 32 31)" fill="none" stroke="#fff" stroke-opacity=".52" stroke-width="1.35"/>`;
  return `<span class="protocol-option-icon proto-3d" aria-hidden="true"><svg class="static-icon" viewBox="0 0 64 64" xmlns="http://www.w3.org/2000/svg"><defs><linearGradient id="${gid}" x1="0" y1="0" x2="1" y2="1"><stop offset="0" stop-color="${a}"/><stop offset=".48" stop-color="${b}"/><stop offset="1" stop-color="${c}"/></linearGradient><radialGradient id="${gid}-glow"><stop offset="0" stop-color="${a}" stop-opacity=".38"/><stop offset="1" stop-color="${a}" stop-opacity="0"/></radialGradient></defs><circle cx="32" cy="36" r="23" fill="url(#${gid}-glow)"/><ellipse cx="32" cy="56" rx="15" ry="3" fill="${b}" opacity=".18"/><path d="M8 39c4-10 12-17 24-19 12-2 21 1 25 9" fill="none" stroke="#fff" stroke-opacity=".10" stroke-width="2"/>${shape}${orbit}<circle cx="52" cy="22" r="2.2" fill="#fff" opacity=".86"/><path d="M9 35c2-6 6-10 11-13" fill="none" stroke="#fff" stroke-opacity=".42" stroke-width="1.2"/><text x="32" y="39" text-anchor="middle" fill="#fff" font-family="Arial,sans-serif" font-size="${safe.length>1?8.5:14}" font-weight="900" style="paint-order:stroke;stroke:#000;stroke-opacity:.16;stroke-width:1">${safe}</text></svg></span>`;
}
