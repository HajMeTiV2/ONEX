...
.bottom-bulk{position:fixed;left:0;right:0;bottom:0;z-index:400;display:none;padding:10px 12px;background:rgba(8,18,35,.94);border-top:1px solid rgba(96,165,250,.22);box-shadow:0 -12px 35px rgba(0,0,0,.28);backdrop-filter:blur(16px)}
.bottom-bulk.show{display:block;animation:bulkBarIn .18s ease-out}
@keyframes bulkBarIn{from{transform:translateY(12px);opacity:0}to{transform:translateY(0);opacity:1}}
.bottom-bulk-inner{max-width:960px;margin:0 auto;display:flex;flex-wrap:wrap;gap:8px;align-items:center;justify-content:center}
.bulk-selected-count{display:inline-flex;align-items:center;gap:6px;padding:8px 11px;border:1px solid rgba(96,165,250,.18);border-radius:10px;background:rgba(37,99,235,.08);color:var(--t1);font-size:12px;font-weight:800}
.bulk-selected-count i{display:block;width:7px;height:7px;border-radius:50%;background:#22c55e;box-shadow:0 0 10px rgba(34,197,94,.55)}
...
.bulk-delete-btn{background:rgba(239,68,68,.12)!important;border-color:rgba(239,68,68,.32)!important;color:#ff8d98!important;font-weight:800!important}
.bulk-delete-btn:hover{background:rgba(239,68,68,.2)!important}
.bulk-clear-btn{opacity:.9}

/* Mobile-friendly config selection: the checkbox stays visible while the wide table scrolls. */
table th:first-child, table td:first-child{overflow:visible}
#page-configs table th:first-child, #page-configs table td:first-child{position:sticky;right:0;z-index:6;background:var(--card);box-shadow:-7px 0 14px rgba(0,0,0,.14)}
#page-configs table thead th:first-child{z-index:8}
...
<div id="bottomBulkBar" class="bottom-bulk" aria-live="polite">
  <div class="bottom-bulk-inner">
    <span id="bulkCount" class="bulk-selected-count"><i></i>0 انتخاب‌شده</span>
    <select id="bulkGroup" aria-label="گروه کانفیگ"></select>
    <button class="btn btn-sm" onclick="bulkMoveGroup()">انتقال به گروه</button>
    <button class="btn btn-sm bulk-delete-btn" onclick="bulkDelete()">🗑 حذف انتخاب‌شده</button>
    <button class="btn btn-sm bulk-clear-btn" onclick="clearSelection()">لغو انتخاب</button>
  </div>
</div>
</body>
</html>
...
async def logout_page(
    ...
    # The sidebar uses a normal GET link. Invalidate the current session
    # and redirect to the login page so refresh cannot reopen the panel.
...
async def create_link_api(
    ...
    if all_protocols:
        config_count = 1
    auto_sub_id = body.get("sub_id")
    auto_sub = None
    if all_protocols and not auto_sub_id:
        auto_sub_id, auto_sub = await create_sub_group(
            name=(str(body.get("label") or "همه پروتکل‌ها")[:50]),
            desc="یک اکانت با همه پروتکل‌های پنل",
        )
    ...
    if all_protocols and auto_sub:
        result["sub_url"] = f"https://{host}/sub-group/{auto_sub["uuid_key"]}"
...
async def create_auto_link(
    ...
    uid, link = await make_link(
        ...
        config_count=config_count, sub_id=auto_sub_id, all_protocols=all_protocols,
    )
...
async def subscription_single(
    ...
    protocols_for_sub = list(PROTOCOLS) if link.get("all_protocols") else [link.get("protocol", DEFAULT_PROTOCOL)]
    if clean_ips:
        ...
        for cip in hosts:
            for proto in protocols_for_sub:
                name = project_config_name(used_names)
                used_names.add(name)
                lines.append(generate_vless_link(uuid, cip, remark=name, protocol=proto, fingerprint=link.get("fingerprint", DEFAULT_FINGERPRINT), alpn=DEFAULT_ALPN_BY_PROTOCOL.get(proto, link.get("alpn")), port=link.get("port", DEFAULT_PORT)))
    else:
        for i in range(cfg_count):
            for proto in protocols_for_sub:
                name = project_config_name(used_names)
                used_names.add(name)
                lines.append(generate_vless_link(uuid, host, remark=name, protocol=proto, fingerprint=link.get("fingerprint", DEFAULT_FINGERPRINT), alpn=DEFAULT_ALPN_BY_PROTOCOL.get(proto, link.get("alpn")), port=link.get("port", DEFAULT_PORT)))
            ...
            if (
                ...
                if link.get("all_protocols"):
                    for proto in PROTOCOLS:
                        variant = dict(link)
                        variant["protocol"] = proto
                        variant["protocol_label"] = PROTOCOL_LABELS.get(proto, proto)
                        variant["alpn"] = DEFAULT_ALPN_BY_PROTOCOL.get(proto, variant.get("alpn", ""))
                        lines.append(vless_link_for_link(variant, link_id, host))
                else:
                    lines.append(
                        vless_link_for_link(
                            link,
...
<section class="page" id="page-configs">
  <div class="page-head">
    <div>
      ...
      <button class="btn btn-sm" onclick="toggleSelectAll(!document.getElementById('chkAll')?.checked)" title="انتخاب همه کانفیگ‌ها">☑ <span class="hide-mobile">انتخاب همه</span></button>
      ...
      <table>
        <thead><tr>
          <th style="width:40px;text-align:center;padding:10px 8px">
            <input type="checkbox" id="chkAll" onchange="toggleSelectAll(this.checked)" title="انتخاب همه" aria-label="انتخاب همه کانفیگ‌ها" style="width:17px;height:17px;margin:0;vertical-align:middle;cursor:pointer">
          </th>
...
<div class="form-row">
        ...
        <label style="display:flex;align-items:center;justify-content:space-between;gap:12px;margin:10px 0 14px;padding:12px 14px;border:1px solid rgba(255,255,255,.08);border-radius:14px;background:rgba(255,255,255,.025);cursor:pointer"><span><b style="display:block;font-size:12px">ساخت همه پروتکل‌ها باهم</b><small style="color:var(--t3)">یک اکانت با همه پروتکل‌های پنل و یک ساب</small></span><input id="cAllProtocols" type="checkbox" style="accent-color:#22c55e;width:20px;height:20px"></label>
    ...
    <div class="card" style="border-color:rgba(139,92,246,.35)">
      ...
      <label style="display:flex;align-items:center;justify-content:space-between;gap:12px;margin:10px 0 14px;padding:12px 14px;border:1px solid rgba(255,255,255,.08);border-radius:14px;background:rgba(255,255,255,.025);cursor:pointer"><span><b style="display:block;font-size:12px">ساخت همه پروتکل‌ها باهم</b><small style="color:var(--t3)">یک اکانت با همه پروتکل‌های پنل و یک ساب</small></span><input id="aAllProtocols" type="checkbox" style="accent-color:#22c55e;width:20px;height:20px"></label>
...
function updateBulkBar(){
  const checks=[...document.querySelectorAll('.cfg-chk')];
  const n=checks.filter(c=>c.checked).length;
  const total=checks.length;
  const bar=document.getElementById('bottomBulkBar');
  const cnt=document.getElementById('bulkCount');
  if(cnt) cnt.innerHTML = `<i></i>${n}` + (lang==='fa'?' کانفیگ انتخاب‌شده':' selected');
...
async function bulkDelete(){
  ...
  const msg=lang==='fa'
    ? `⚠️ ${ids.length} کانفیگ انتخاب شده است.\n\nبا حذف، کانفیگ‌ها و لینک سابسکریپشن آن‌ها از پنل پاک می‌شوند.\nاین عملیات قابل بازگشت نیست.\n\nادامه می‌دهید؟`
    : `⚠️ ${ids.length} configs selected.\n\nThey and their subscription links will be removed from the panel.\nThis action cannot be undone.\n\nContinue?`;
  if(!confirm(msg))return;
  const r=await api('/api/links/bulk-delete',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({ids})});
  if(r){
    clearSelection();
    toast(lang==='fa'?`✓ ${r.deleted} کانفیگ حذف شد`:`✓ ${r.deleted} configs deleted`);
    refreshAll();
  }
}
...
async function loadGroups(){
  ...
  const opts=list.map(g=>`<option value="${esc(g.id)}">