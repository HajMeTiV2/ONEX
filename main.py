...
<div class="onex-topbar">
  ...
  <div class="top-actions">
    <div class="top-setting-group" aria-label="Theme controls">
      <button type="button" class="top-setting-btn" id="topThemeDark" onclick="setTheme('dark')" title="تم تیره">☾ <span>تیره</span></button>
      <button type="button" class="top-setting-btn" id="topThemeLight" onclick="setTheme('light')" title="تم روشن">☀ <span>روشن</span></button>
    </div>
...
function updateTopControls(){
  ...
  const td=document.getElementById('topThemeDark'),tl=document.getElementById('topThemeLight'),lf=document.getElementById('topLangFa'),le=document.getElementById('topLangEn');
  if(td){td.classList.toggle('active',!light);const x=td.querySelector('span');if(x)x.textContent=lang==='fa'?'تیره':'Dark'}
  if(tl){tl.classList.toggle('active',light);const x=tl.querySelector('span');if(x)x.textContent=lang==='fa'?'روشن':'Light'}
...
function setTheme(mode){mode=mode==='light'?'light':'dark';document.documentElement.classList.toggle('light',mode==='light');document.body.classList.toggle('light-theme',mode==='light');localStorage.setItem('px_theme',mode);updateTopControls()}
(function(){const th=localStorage.getItem('px_theme')||'dark';setTheme(th);applyLang()})();
