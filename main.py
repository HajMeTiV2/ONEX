...
async def api_update_deploy(token=Depends(require_auth)):
    ...
    try:
        ...
        deployment = ((data.get("data") or {}).get("serviceInstanceDeployV2") or {})
...
async function checkPanelUpdate(showToast=true){
  ...
  try{
    ...
    if(r&&r.ok){
      __updateInfo=r;
      if(r.update_available&&showToast){
        toast(updateText(`نسخه جدید ${r.latest_version} آماده است`, `Version ${r.latest_version} is available`));
      }
    }
    return r;
  }catch(e){return null}
