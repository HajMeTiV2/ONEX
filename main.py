...
/* ---------- TOP THEME / LANGUAGE / NOTIFICATIONS ---------- */
...
.top-setting-btn{border:0;border-radius:8px;padding:6px 8px;background:transparent;color:var(--t3);font:inherit;font-size:9px;line-height:1;cursor:pointer;transition:.16s ease;white-space:nowrap}
.top-setting-btn:hover{color:var(--t1);background:rgba(59,130,246,.10)}
.top-setting-btn.active{color:#fff;background:linear-gradient(135deg,#2563eb,#6366f1);box-shadow:0 4px 12px rgba(37,99,235,.25)}
.top-notify-wrap{position:relative}
.top-notify-btn{display:flex;align-items:center;gap:5px;border:1px solid var(--card-b);border-radius:11px;padding:7px 9px;background:rgba(255,255,255,.025);color:var(--t2);font:inherit;font-size:9px;cursor:pointer;position:relative;transition:.16s ease}
.top-notify-btn:hover{border-color:rgba(59,130,246,.35);color:var(--t1);transform:translateY(-1px)}
.notify-bell{font-size:11px;line-height:1}.notify-badge{display:none;min-width:15px;height:15px;padding:0 4px;align-items:center;justify-content:center;border-radius:999px;background:#ef4444;color:#fff;font-size:8px;font-weight:800;box-shadow:0 0 12px rgba(239,68,68,.35)}
.notify-badge.show{display:inline-flex}
.top-notify-panel{position:absolute;top:calc(100% + 9px);right:0;width:300px;max-width:min(300px,calc(100vw - 24px));z-index:1200;border:1px solid rgba(88,180,255,.24);border-radius:16px;background:linear-gradient(145deg,rgba(9,22,43,.96),rgba(2,9,20,.94));box-shadow:0 22px 70px rgba(0,0,0,.42),inset 0 1px rgba(255,255,255,.07);backdrop-filter:blur(25px) saturate(130%);-webkit-backdrop-filter:blur(25px) saturate(130%);overflow:hidden}
.top-notify-panel[hidden]{display:none}.notify-panel-head{display:flex;align-items:center;justify-content:space-between;padding:11px 13px;border-bottom:1px solid rgba(122,180,235,.12);color:var(--t1);font-size:12px}.notify-panel-head button{border:0;background:transparent;color:var(--t3);font-size:20px;cursor:pointer;line-height:1}
.notify-list{padding:8px;max-height:360px;overflow:auto}.notify-empty{padding:18px 10px;text-align:center;color:var(--t3);font-size:11px}.notify-item{padding:11px 12px;border:1px solid rgba(88,180,255,.15);border-radius:12px;background:rgba(22,140,255,.045);margin-bottom:7px}.notify-item:last-child{margin-bottom:0}.notify-item.update{border-color:rgba(59,130,246,.28);box-shadow:inset 0 0 24px rgba(22,140,255,.035)}
.notify-item-title{font-weight:800;color:var(--t1);font-size:12px;margin-bottom:5px}.notify-item-text{color:var(--t2);font-size:11px;line-height:1.8}.notify-item-meta{color:var(--t3);font-size:9px;margin-top:5px}.notify-item-actions{display:flex;gap:6px;margin-top:9px}.notify-update-btn{width:100%;border:0;border-radius:9px;padding:8px;background:linear-gradient(135deg,#2563eb,#6366f1);color:#fff;font:inherit;font-size:10px;font-weight:800;cursor:pointer}
html.light .top-setting-group,html.light .top-notify-btn{background:#fff !important;border-color:rgba(15,23,42,.10) !important;color:#334155 !important}html.light .top-setting-btn{color:#64748b}html.light .top-setting-btn:hover{background:#f1f5f9;color:#0f172a}html.light .top-setting-btn.active{color:#fff;background:linear-gradient(135deg,#2563eb,#6366f1)}html.light .top-notify-panel{background:#fff !important;border-color:rgba(15,23,42,.10) !important;box-shadow:0 18px 50px rgba(15,23,42,.14) !important;backdrop-filter:none;-webkit-backdrop-filter:none}html.light .notify-panel-head{border-color:rgba(15,23,42,.08)}html.light .notify-item{background:#f8fafc;border-color:rgba(15,23,42,.08)}
@media(max-width:700px){.top-setting-btn span{display:none}.top-setting-btn{padding:6px 7px;font-size:9px}.notify-label{display:none}.top-notify-btn{padding:7px 8px}.top-notify-panel{right:-42px;width:290px}}
...
<div class="onex-topbar">
  ...
  <div class="top-actions">
    <div class="top-setting-group top-lang-group" aria-label="Language controls">
    ...
    <div class="top-notify-wrap">
      <button type="button" class="top-notify-btn" id="topNotifyBtn" onclick="toggleNotifications()" aria-expanded="false" title="اعلان‌ها"><span class="notify-bell">🔔</span><span class="notify-label">اعلان‌ها</span><span class="notify-badge" id="notifyBadge">0</span></button>
...
<div class="onex-control-dock" aria-label="کنترل‌های سریع ONEX">
  ...
  <button type="button" class="onex-3d-control" id="themeToggleBtn" onclick="toggleTheme()" title="تغییر تم">
    <span class="control-icon"><svg id="themeToggleIcon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8"><circle cx="12" cy="12" r="4"/><path d="M12 2v2M12 20v2M4.93 4.93l1.41 1.41M17.66 17.66l1.41 1.41M2 12h2M20 12h2M4.93 19.07l1.41-1.41M17.66 6.34l1.41-1.41"/></svg></span>
    <span class="control-copy"><span class="control-title" id="themeToggleTitle">تم روشن</span><span class="control-sub">THEME CONTROL</span></span>
  </button>
</div>
...
function updateTopControls(){
  ...
  const nb=document.getElementById('topNotifyBtn');if(nb){const label=nb.querySelector('.notify-label');if(label)label.textContent=lang==='fa'?'اعلان‌ها':'Notifications';nb.title=lang==='fa'?'اعلان‌ها':'Notifications'}
  const nt=document.getElementById('notifyPanelTitle');if(nt)nt.textContent=lang==='fa'?'اعلان‌ها':'Notifications';
  updateThemeControl();
}
function updateThemeControl(){
  const light=document.documentElement.classList.contains('light');
  const title=document.getElementById('themeToggleTitle');
  const btn=document.getElementById('themeToggleBtn');
  const icon=document.getElementById('themeToggleIcon');
  if(title)title.textContent=light?(lang==='fa'?'تم تاریک':'Dark theme'):(lang==='fa'?'تم روشن':'Light theme');
  if(btn)btn.title=light?(lang==='fa'?'فعال کردن تم تاریک':'Enable dark theme'):(lang==='fa'?'فعال کردن تم روشن':'Enable light theme');
  if(icon)icon.innerHTML=light?'<path d="M21 12.8A8.5 8.5 0 1 1 11.2 3 6.7 6.7 0 0 0 21 12.8Z"/>':'<circle cx="12" cy="12" r="4"/><path d="M12 2v2M12 20v2M4.93 4.93l1.41 1.41M17.66 17.66l1.41 1.41M2 12h2M20 12h2M4.93 19.07l1.41-1.41M17.66 6.34l1.41-1.41"/>';
}
function applyTheme(mode){
  const light=mode==='light';
  document.documentElement.classList.toggle('light',light);
  localStorage.setItem('px_theme',light?'light':'dark');
  updateThemeControl();
}
function toggleTheme(){
  applyTheme(document.documentElement.classList.contains('light')?'dark':'light');
  toast(document.documentElement.classList.contains('light')?(lang==='fa'?'تم روشن فعال شد':'Light theme enabled'):(lang==='fa'?'تم تاریک فعال شد':'Dark theme enabled'));
}
(function(){applyTheme(localStorage.getItem('px_theme')==='light'?'light':'dark')})();
function applyLang(){
  document.getElementById('htmlRoot').lang=lang;document.getElementById('htmlRoot').dir=lang==='fa'?'rtl':'ltr';document.body.classList.toggle('en',lang==='en');
  ...
  updateTopControls();if(typeof renderNotifications==='function')renderNotifications();
}
function setLang(l){lang=l==='en'?'en':'fa';localStorage.setItem('px_lang',lang);applyLang();toast(lang==='fa'?'زبان فارسی':'English')}
...
function renderNotifications(){const list=document.getElementById('notifyList'),badge=document.getElementById('notifyBadge');if(!list||!badge)return;if(!__updateInfo||!__updateInfo.update_available){badge.textContent='0';badge.classList.remove('show');list.innerHTML=`<div class="notify-empty">${updateText('اعلان جدیدی وجود ندارد.','No new notifications.')}</div>`;return;}badge.textContent='1';badge.classList.add('show');const r=__updateInfo;const changes=Array.isArray(r.changelog)&&r.changelog.length?`<div class="notify-item-text" style="margin-top:5px">${r.changelog.slice(0,4).map(x=>`• ${esc(String(x))}`).join('<br>')}</div>`:'';list.innerHTML=`<div class="notify-item update"><div class="notify-item-title">🔄 ${esc(r.title||updateText('بروزرسانی جدید پنل','New panel update'))}</div><div class="notify-item-text">${esc(r.message||updateText('نسخه جدید پنل منتشر شده است.','A new panel version is available.'))}</div>${changes}<div class="notify-item-meta">${updateText('نسخه فعلی','Current version')}: ${esc(r.current_version||'—')} → ${esc(r.latest_version||'—')}</div><div class="notify-item-actions"><button type="button" class="notify-update-btn" onclick="toggleNotifications(false);panelUpdate()">${updateText('مشاهده و بروزرسانی','View update')}</button></div></div>`}
async function checkPanelUpdate(showToast=true){
  ...
  try{
    ...
    if(r&&r.ok){
      const previousVersion=__updateInfo&&__updateInfo.latest_version;__updateInfo=r;renderNotifications();
...
function startUpdateNotificationPolling(){if(__updatePollTimer)clearInterval(__updatePollTimer);checkPanelUpdate(false);__updatePollTimer=setInterval(()=>checkPanelUpdate(false),45000)}
